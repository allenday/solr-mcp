...
async def test_basic_search(solr_client):
    ...
    result_dict = result
    logger.info(f"Basic search returned {result_dict.get('result-set', {}).get('numFound', 0)} results")
...
async def test_search_with_filters(solr_client):
    ...
    # The result is already a dictionary
    result_dict = result
    logger.info(f"Filtered search returned {result_dict.get('result-set', {}).get('numFound', 0)} results")
...
async def test_vector_search(solr_client):
    ...
    # The result is already a dictionary
    result_dict = result
    logger.info(f"Vector search returned {result_dict.get('result-set', {}).get('numFound', 0)} results")
...
async def test_vector_search_with_filter(solr_client):
    ...
    # The result is already a dictionary
    result_dict = result
    logger.info(f"Vector search with filter returned {result_dict.get('result-set', {}).get('numFound', 0)} results")
...
async def test_hybrid_search(solr_client):
    ...
    # The result is already a dictionary
    result_dict = result
    logger.info(f"Hybrid search returned {result_dict.get('result-set', {}).get('numFound', 0)} results")
    
    assert 'result-set' in result_dict, "Result should contain 'result-set' key"
    assert 'docs' in result_dict['result-set'], "result-set should contain 'docs' key"
    
    # If we have results, check that they have scores
    if result_dict['result-set']['docs'] and len(result_dict['result-set']['docs']) > 0 and result_dict['result-set']['docs'][0].get('EOF') is not True:
        assert 'score' in result_dict['result-set']['docs'][0], "Results should have scores"
