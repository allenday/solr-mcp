...
import pytest
from solr_mcp.solr.vector import VectorManager, VectorSearchResults, VectorSearchResult
...
class TestVectorManager:
    ...
    def test_init(self, mock_ollama, mock_solr_instance):
        ...
        manager = VectorManager(solr=mock_solr_instance, client=mock_ollama)
        assert manager.client == mock_ollama
        assert manager.solr == mock_solr_instance
    ...
    def test_init_no_client(self, mock_solr_instance):
        ...
        manager = VectorManager(solr=mock_solr_instance)
        assert manager.client is not None  # Should create default OllamaClient
        assert manager.solr == mock_solr_instance
    ...
    def test_format_knn_query(self, mock_ollama, mock_solr_instance):
        ...
        assert query == "{!knn f=embedding topK=10}[0.1,0.2,0.3]"
        
        # Test with custom top_k
        query = manager.format_knn_query(vector, top_k=5)
        assert query == "{!knn f=embedding topK=5}[0.1,0.2,0.3]"

    def test_execute_vector_search_success(self, mock_ollama, mock_solr_instance):
        ...
        mock_solr_instance.search.return_value = {
            ...
            "response": {
                "docs": [{"id": "1", "score": 0.95, "_vector_distance_": 0.05}],
                "numFound": 1,
                "maxScore": 0.95
            }
        }
        ...
        results = manager.execute_vector_search(mock_solr_instance, vector)
        mock_solr_instance.search.assert_called_with(
            "{!knn f=embedding topK=10}[0.1,0.2,0.3]",
            fq=None,
            fl="id,score,embedding"
        )
        assert results["response"]["docs"][0]["score"] == 0.95
        
        # Test with filter query
        filter_query = "type:document"
        results = manager.execute_vector_search(mock_solr_instance, vector, filter_query=filter_query)
        mock_solr_instance.search.assert_called_with(
            "{!knn f=embedding topK=10}[0.1,0.2,0.3]",
            fq=filter_query,
            fl="id,score,embedding"
    ...
    def test_execute_vector_search_error(self, mock_ollama, mock_solr_instance):
        ...
        with pytest.raises(SolrError, match="Vector search failed"):
            manager.execute_vector_search(mock_solr_instance, vector)
...
class TestVectorSearchResults:
    ...
    def test_get_doc_ids(self):
        ...
        results = VectorSearchResults(
            results=[
                VectorSearchResult(id="1", score=0.95),
                VectorSearchResult(id="2", score=0.85)
            ],
            total_found=2,
            top_k=2
        )
