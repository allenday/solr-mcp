...
def test_get_field_info_success(field_manager, mock_schema_response):
    ...
    with patch.object(field_manager, 'get_schema', return_value=mock_schema_response["schema"]):
        ...
        assert set(field_info["sortable_fields"].keys()) >= {"id", "sort_field", "_docid_", "score"}
...
def test_get_field_info_nonexistent_field(field_manager, mock_schema_response):
    ...
    with patch.object(field_manager, 'get_schema', return_value=mock_schema_response["schema"]):
        with pytest.raises(SchemaError, match="Field nonexistent not found in collection test_collection"):
...
def test_get_field_info_no_fields(field_manager):
    ...
    with patch.object(field_manager, 'get_schema', return_value={"schema": {}}):
        ...
        assert field_info["sortable_fields"] == {"_docid_": {"type": "numeric", "searchable": False, "directions": ["asc", "desc"], "default_direction": "asc"},
                                                "score": {"type": "numeric", "searchable": True, "directions": ["asc", "desc"], "default_direction": "desc"}}
...
def test_validate_field_exists_error(field_manager):
    ...
    with patch.object(field_manager, 'get_field_info', side_effect=Exception("Test error")):
        with pytest.raises(SchemaError, match="Error validating field test: Test error"):
...
def test_validate_sort_field_success(field_manager):
    ...
    with patch.object(field_manager, 'get_field_info') as mock_get_info:
        ...
        assert field_manager.validate_sort_field("sort_field", "test_collection") is True

def test_validate_sort_field_not_found(field_manager):
    """Test validating non-sortable field."""
    with patch.object(field_manager, 'get_field_info') as mock_get_info:
        mock_get_info.return_value = {"sortable_fields": {"sort_field": {}, "id": {}}}
        with pytest.raises(SchemaError, match="Field title is not sortable in collection test_collection"):
...
def test_validate_sort_field_error(field_manager):
    ...
    with patch.object(field_manager, 'get_field_info', side_effect=Exception("Test error")):
        with pytest.raises(SchemaError, match="Error validating sort field test: Test error"):
...
def test_get_field_types_cached(field_manager, mock_schema_response):
    ...
    with patch.object(field_manager, 'get_schema', return_value=mock_schema_response["schema"]) as mock_get_schema:
...
def test_validate_collection_exists_error(field_manager):
    ...
    with patch.object(field_manager, 'get_schema', side_effect=Exception("Test error")):
        with pytest.raises(SchemaError, match="Error validating collection: Test error"):
...
def test_validate_fields_error(field_manager):
    ...
    with patch.object(field_manager, '_get_collection_fields') as mock_get_fields:
        ...
        with pytest.raises(SchemaError, match="Invalid fields for collection test_collection: nonexistent"):
...
def test_validate_sort_fields_error(field_manager):
    ...
    with patch.object(field_manager, '_get_collection_fields') as mock_get_fields:
        ...
        with pytest.raises(SchemaError, match="Fields not sortable in collection test_collection: title"):
            field_manager.validate_sort_fields("test_collection", ["sort_field", "title"]) 