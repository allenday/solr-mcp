...
from solr_mcp.solr.utils.formatting import format_search_results, format_sql_response, format_error_response
...
def test_format_search_results_with_highlighting(mock_results):
    ...
    formatted = json.loads(format_search_results(mock_results, include_highlighting=True))
    assert "highlighting" in formatted["result-set"]
    assert formatted["result-set"]["highlighting"] == {"1": {"title": ["<em>Test</em>"]}}
...
def test_format_search_results_without_optional_fields(mock_results):
    ...
    formatted = json.loads(format_search_results(
        mock_results,
        include_score=False,
