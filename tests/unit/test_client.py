...
from .conftest import MockCollectionProvider, MockVectorProvider, MOCK_RESPONSES
...
class TestSolrClient:
    ...
    async def test_execute_select_query_success(self, mock_config, mock_field_manager, collection):
        ...
        mock_query_builder.parser.preprocess_query = Mock(return_value=f"SELECT * FROM {collection}")
        mock_query_builder.parse_and_validate_select = Mock(return_value=(
            Mock(),  # AST
