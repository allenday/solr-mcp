...
def mock_http_response(request):
    ...
    response_data = getattr(request, 'param', {}).get('response_data', MOCK_RESPONSES["select"])
...
def mock_http_client(request):
    ...
    fields_response = params.get('fields_response', {"fields": MOCK_RESPONSES["schema"]["schema"]["fields"]})
...
def mock_solr_requests(mock_http_client):
    ...
    with patch("requests.post", mock_http_client.post), \
...
def mock_aiohttp_session(request):
    ...
    response_data = getattr(request, 'param', {}).get('response_data', 
        '{"result-set": {"docs": [{"id": "1"}], "numFound": 1}}')
    vector_response = getattr(request, 'param', {}).get('vector_response', {
        'response': {
    ...
    if error:
        ...
        mock_response.__aenter__ = AsyncMock(side_effect=Exception("Mock session error"))
    else:
