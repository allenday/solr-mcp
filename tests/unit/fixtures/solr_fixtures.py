...
def mock_collection_provider(request):
    ...
    """
    # Get parameters or use defaults
    collections = getattr(request, 'param', {}).get('collections', MOCK_RESPONSES["collections"])
...
def mock_field_manager(request):
    ...
    """
    # Get parameters or use defaults
    fields = getattr(request, 'param', {}).get('fields', MOCK_RESPONSES["field_list"]["fields"])
    error = getattr(request, 'param', {}).get('error', False)
    
    manager = MagicMock()
    
    if error:
        manager.get_collection_fields = Mock(side_effect=SolrError("Failed to retrieve fields"))
    else:
...
def mock_solr_client(request):
    ...
    # Get parameters or use defaults
    error = getattr(request, 'param', {}).get('error', False)
    select_response = getattr(request, 'param', {}).get('select_response', MOCK_RESPONSES["select"])
    vector_response = getattr(request, 'param', {}).get('vector_response', MOCK_RESPONSES["vector"])
    semantic_response = getattr(request, 'param', {}).get('semantic_response', MOCK_RESPONSES["semantic"])
    collections = getattr(request, 'param', {}).get('collections', MOCK_RESPONSES["collections"])
    fields = getattr(request, 'param', {}).get('fields', MOCK_RESPONSES["field_list"]["fields"])
