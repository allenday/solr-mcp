...
@pytest.mark.parametrize("collection,info", [
    ("collection1", SAMPLE_FIELD_INFO),
    ("test_collection", {"searchable_fields": ["title"], "sortable_fields": {"id": {"directions": ["asc"], "default_direction": "asc"}}}),
])
class TestFieldCacheBasic:
    ...
    def test_set(self, field_cache, collection, info):
        ...
        assert field_cache._cache[collection]["searchable_fields"] == info["searchable_fields"]
        assert field_cache._cache[collection]["sortable_fields"] == info["sortable_fields"]
...
class TestFieldCacheOperations:
    ...
    @pytest.mark.parametrize("time_values,max_age,expected_stale", [
    ...
    def test_is_stale_with_time(self, field_cache, patch_module, time_values, max_age, expected_stale):
        ...
        with patch_module('time.time', side_effect=time_values):
            ...
            if max_age == 300:  # Default max_age
                ...
                assert field_cache.is_stale("collection1", max_age=max_age) is expected_stale
    ...
    @pytest.mark.parametrize("update_info", [
        {"searchable_fields": ["new_field"]},
        {"sortable_fields": {"new_id": {"directions": ["asc"], "default_direction": "asc"}}},
