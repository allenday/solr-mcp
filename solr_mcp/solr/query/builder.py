...
from sqlglot.expressions import Select, From, Column, Identifier, Where, Binary, Ordered, Star, Literal, EQ
...
class QueryBuilder:
    ...
    def parse_and_validate(self, query: str) -> tuple[Select, str, list[str], list[tuple[str, str]]]:
        ...
        if "*" not in fields:
            for field in fields:
                if not self.field_manager.validate_field_exists(field, collection):
                    raise QueryError(f"Field '{field}' does not exist in collection '{collection}'")
        ...
        if sort_fields:
            for field, direction in sort_fields:
                if not self.field_manager.validate_field_exists(field, collection):
                    raise QueryError(f"Sort field '{field}' does not exist in collection '{collection}'")
                if not self.field_manager.validate_sort_field(field, collection):
                    raise QueryError(f"Field '{field}' is not sortable in collection '{collection}'")
    ...
    def validate_sort(self, sort_spec: str | None, collection: str) -> str | None:
        ...
        try:
            ...
            if direction not in ["ASC", "DESC"]:
                raise QueryError(f"Invalid sort direction '{direction}'. Must be ASC or DESC")
    ...
    def _convert_where_to_solr(self, where_expr: exp.Expression) -> str:
        ...
        if isinstance(where_expr, Where):
            ...
            return where_expr.args['this'].name if 'this' in where_expr.args else where_expr.name
        elif isinstance(where_expr, Literal):
            if where_expr.is_string:
                return f'"{where_expr.this}"'
            return str(where_expr.this)
        else:
            raise QueryError(f"Unsupported expression type '{type(where_expr).__name__}' in WHERE clause")
    ...
    def build_solr_query(self, ast: Select) -> Dict[str, Any]:
        ...
        if ast.expressions and not isinstance(ast.expressions[0], Star):
            params["fl"] = ",".join(expr.args['this'].name if isinstance(expr, Column) else str(expr) for expr in ast.expressions)
        ...
        sort_fields = self.parser.get_sort_fields(ast)
        if sort_fields:
            params["sort"] = ",".join(f"{field} {direction}" for field, direction in sort_fields)
